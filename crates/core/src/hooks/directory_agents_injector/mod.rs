...
use std::fs;
use crate::hooks::directory_agents_injector::constants::AGENTS_FILENAME;
use crate::hooks::directory_agents_injector::storage::{load_injected_paths, save_injected_paths, clear_injected_paths};
...
impl DirectoryAgentsInjectorHook {
    ...
    pub async fn process_file_for_injection(
        ...
        for path in agents_paths {
            let agents_dir = path.parent().unwrap().to_str().unwrap_or("").to_string();
            if cache.contains(&agents_dir) { continue; }
