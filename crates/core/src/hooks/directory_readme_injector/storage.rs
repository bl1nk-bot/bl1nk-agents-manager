...
use chrono::Utc;
use crate::hooks::directory_readme_injector::constants::readme_injector_storage;
use crate::hooks::directory_readme_injector::types::InjectedPathsData;

fn get_storage_path(session_id: &str) -> PathBuf {
    readme_injector_storage().join(format!("{}.json", session_id))
}
...
pub fn save_injected_paths(session_id: &str, paths: &HashSet<String>) -> anyhow::Result<()> {
    let storage_dir = readme_injector_storage();
