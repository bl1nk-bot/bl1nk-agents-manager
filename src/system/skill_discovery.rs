...
fn process_and_validate(
    ...
    let raw: SkillFrontmatter = serde_json::from_value(frontmatter_json)
        .context("ไม่สามารถแปลงข้อมูลเป็น SkillFrontmatter ได้")?;
    ...
    if seen_names.contains(&normalized) { return Err(anyhow!("พบชื่อซ้ำ: {}", raw.name)); }
    ...
    Ok(SkillMetadata {
        name, description, path: base_path.to_path_buf(),
...
pub fn extract_skill_body(content: &str) -> String {
    let re = Regex::new(r"(?s)^---\r?\n([\s\S]*?)\r?\n---\r?\n?").unwrap();
    if let Some(m) = re.find(content) { content[m.end()..].trim().to_string() } else { content.trim().to_string() }
}
